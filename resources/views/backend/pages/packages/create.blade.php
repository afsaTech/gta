...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="my-1 float-left"><i class="fas fa-info-circle text-blue"></i>&nbsp;{{ 'Add New Package' }}</h3>
                <div class="btn-group btn-group-sm float-right" role="group">
                    @can('categories.create')
                        <a href="{{ route('categories.create') }}" class="btn btn-outline-success" title="Add New Category">
                            <i class=" fas fa-fw fa-plus-circle" aria-hidden="true"></i> Add Category
                        </a>
                    @endcan
                    @can('package-images.create')
                        <a href="{{ route('package-images.create') }}" class="btn btn-outline-dark" title="Add New Image">
        ...
        <div class="card-body">
            <form action="{{ route('packages.store') }}" method="post" class="form" enctype="multipart/form-data">
                @csrf
